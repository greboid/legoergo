...
WORKDIR /app
RUN git clone --no-tags --branch v2.12.0 --single-branch --depth 1 https://github.com/ergochat/ergo .
...
WORKDIR /app
RUN git clone --no-tags --branch v4.2.0 --single-branch --depth 1 https://github.com/csmith/certwrapper .
RUN CGO_ENABLED=0 GOOS=linux go build -trimpath -gcflags=./dontoptimizeme=-N -ldflags=-s -o /app/main .
